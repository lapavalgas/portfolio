...
function scrollFunction() {
    ...
    if (isUserInCoverComponent()) { footerTransformToHidden(); logoTransformOpacityZero(); } else { footerTransformToVisible(); logoTransformOpacityOne(); }
    // to merge footer to contact component 
    if (isUserInFooterComponent()) { footerTransformToContactComponent(); logoTransformColorWhite(); menuTransformColorWhite(); } else { footerTransformToFooterComponent(); logoTransformColorBlack(); menuTransformColorBlack(); }
...
function hiddenMenuFunction() {
    let array = document.getElementsByClassName("hidden-menu");
